...
impl DeviceCommunicationPort for ProtobufDeviceCommunicator {
    async fn send_route_summary(&self, device_id: String, session: &NavigationSession) -> Result<()> {
        let summary = proto::RouteSummary {
            ...
            eta_unix_ms: (chrono::Utc::now().timestamp() + session.route.duration_seconds as i64) as u64 * 1000,
    ...
    async fn send_route_blob(&self, device_id: String, route: &Route) -> Result<()> {
        ...
        // Create waypoints
        let waypoints: Vec<proto::Waypoint> = route.waypoints.iter().enumerate().map(|(idx, wp)| {
            proto::Waypoint {
        ...
        let blob = proto::RouteBlob {
            ...
            polyline_data: Some(proto::route_blob::PolylineData::EncodedPolyline(polyline_json)),
            metadata: None,
            compressed: false,
            checksum,
            signature: None,
